...
class LineMapper:
    ...
    def _find_line_by_code_content(
        ...
        for i, diff_line in enumerate(diff_lines):
            ...
            # Check if the first line of our search block is in this diff line
            if search_lines[0] in diff_line:
                # If it's a multi-line block, check subsequent lines
                is_match = True
                for j, search_line in enumerate(search_lines[1:]):
                    next_diff_line_index = i + 1 + j
                    if not (
                        next_diff_line_index < len(diff_lines)
                        and search_line in diff_lines[next_diff_line_index]
                    ):
                        is_match = False
                        break
                ...
                if is_match:
                    ...
                    line_info = parsed_file.position_to_line.get(matched_global_position)
