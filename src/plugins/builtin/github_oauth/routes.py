    ...
    def _setup_routes(self):
        ...
        async def initiate_oauth(request: Request, response: Response):
            ...
            try:
                ...
                return RedirectResponse(url=auth_data['auth_url'], status_code=HTTP_302_FOUND)
        ...
        async def oauth_callback(
            ...
            try:
                ...
                if error:
                    ...
                    return JSONResponse(
                        content={"error": "OAuth authorization failed", "details": error},
                ...
                if state != oauth_state:
                    logger.warning(f"Invalid OAuth state: expected {oauth_state}, got {state}")
                    raise HTTPException(
                        status_code=HTTP_400_BAD_REQUEST,
                ...
                user_info = await self.oauth_handler.get_user_info(token_info['access_token'])
                if not user_info:
                ...
                user = await self.oauth_handler.create_or_update_user(user_info, token_info)
                if not user:
                ...
                repositories = await self.oauth_handler.get_user_repositories(token_info['access_token'])
                if repositories:
                    await self.oauth_handler.update_user_repositories(user, repositories)
        ...
        async def configure_webhook(
            ...
            try:
                ...
                # TODO: Implement webhook configuration via GitHub API
                # This would involve creating a webhook that points to the SourceAnt
                # webhook endpoint for this specific user/repository
        ...
        async def remove_webhook(
            ...
            try:
                ...
                # TODO: Remove webhook via GitHub API
