...
from .webhook_handler import GitHubOAuthWebhookHandler
...
class GitHubOAuthPlugin(BasePlugin):
    ...
    async def _initialize(self) -> None:
        ...
        client_secret = self.get_config("client_secret") or os.getenv("GITHUB_OAUTH_CLIENT_SECRET")
        redirect_uri = self.get_config("redirect_uri", "http://localhost:8000/auth/github/callback")
    ...
    async def _before_webhook_processing(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            db = next(get_db())
            try:
                authorized_repo = db.query(UserRepository).filter(
                ...
                if authorized_repo:
                    logger.info(f"Repository {repository_full_name} is authorized by user")
                    return {
                        'authorized': True,
                    ...
                    return {'authorized': False, 'reason': 'Repository not authorized by any user'}
    ...
    async def _after_user_authentication(self, context: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if access_token:
                repositories = await self.oauth_handler.get_user_repositories(access_token)
                if repositories:
                    count = await self.oauth_handler.update_user_repositories(user, repositories)
