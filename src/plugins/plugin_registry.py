...
class PluginRegistry:
    ...
    def set_plugin_status(self, plugin_name: str, status: PluginStatus, error_message: Optional[str] = None):
        """
    ...
    def update_dependencies_status(self):
        ...
        for plugin_name, plugin_info in self._plugins.items():
            ...
            dependencies_met = all(
                dep_name in self._plugins and 
                self._plugins[dep_name].status in [PluginStatus.STARTED, PluginStatus.INITIALIZED]
        ...
        def visit(plugin_name: str):
            ...
            for dep_name in dependencies:
                if dep_name in self._plugins:  # Only consider registered dependencies
                    ...
                    logger.warning(f"Plugin {plugin_name} depends on unregistered plugin: {dep_name}")
    ...
    def get_plugin_summary(self) -> Dict[str, Dict[str, any]]:
        ...
        for plugin_name, plugin_info in self._plugins.items():
            summary[plugin_name] = {
                ...
                'start_time': plugin_info.start_time.isoformat() if plugin_info.start_time else None,
