...
class EventHooks:
    ...
    def register_hook(
        ...
        if hook_name not in self.HOOK_POINTS:
            raise ValueError(f"Invalid hook name: {hook_name}. Valid hooks: {self.HOOK_POINTS}")
        ...
        logger.info(f"Registered {hook_name} hook for plugin {plugin_name} with priority {priority}")
        return True
    ...
    def unregister_hook(self, hook_name: str, plugin_name: str) -> bool:
        ...
        if removed_count > 0:
            logger.info(f"Unregistered {removed_count} {hook_name} hooks for plugin {plugin_name}")
    ...
    def unregister_plugin_hooks(self, plugin_name: str) -> int:
        ...
        if total_removed > 0:
            logger.info(f"Unregistered {total_removed} total hooks for plugin {plugin_name}")
    ...
    async def broadcast_event(
        ...
        """
        logger.debug(f"Broadcasting event {event_type} from {source_plugin or 'system'}")
        ...
        for registration in subscribers:
            try:
                logger.debug(f"Executing {event_type} handler for plugin {registration.plugin_name}")
                ...
                plugin_stats[registration.plugin_name] = plugin_stats.get(registration.plugin_name, 0) + 1
                
            except Exception as e:
                logger.error(f"Error executing {event_type} handler for plugin {registration.plugin_name}: {e}")
                results[registration.plugin_name] = {'error': str(e)}
        
        logger.debug(f"Event {event_type} broadcast completed with {len(results)} responses")
        return results
