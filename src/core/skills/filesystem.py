...
KNOWN = frozenset(
    {"name", "description", "paths", "metadata", "content", "disable-model-invocation"}
)
...
def skill_from_markdown(
    ...
    content = dict(fields["content"]) if isinstance(fields.get("content"), dict) else {}
    if "content" in fields:
        content["instructions"] = body[:MAX_BODY]
    return Skill(
        id=identifier,
